"""
HL TRADER — Final Production App
══════════════════════════════════
8 assets | ntfy alerts | /test | /force-trade | /signal-check | Tax system
...
# Per-asset validated configs (OOS confirmed)
ASSET_CFG = {
    ...
    "XRP":  {"exit":"trail",    "ff":None,  "bb":False,"sc":True, "no_ov":True, "pt":None,"ps":None,"tp":None,"cd":0, "regime":True},
    ...
    "LINK": {"exit":"partial",  "ff":None,  "bb":True, "sc":True, "no_ov":True, "pt":0.04,"ps":0.25,"tp":None,"cd":0, "regime":True},
}

# Tax rates — Section 1256 (60/40 rule)
FED_LTCG_RATE  = 0.20
FED_STCG_RATE  = 0.37
NY_STATE_RATE  = 0.0685
NYC_LOCAL_RATE = 0.03876
SEC1256_LTCG   = 0.60
SEC1256_STCG   = 0.40
...
def ntfy_api_down():
    ntfy("⚠️ API Offline",
         "HyperLiquid API is not responding\nSystem retrying automatically\nPositions held — no exits managed until recovered",
         priority="high",tags="warning")

def ntfy_api_recovered(down_minutes):
    ntfy("✅ API Recovered",
         f"HyperLiquid back online after {down_minutes:.0f} min\nTrading resumed normally",
         tags="white_check_mark")

def ntfy_kill_switch():
    ntfy("🛑 Kill Switch Activated",
...
def check_tax_reminders():
    ...
    for q in QUARTERLY_DATES:
        ...
        qk=f"{q['due'][:4]}-{q['quarter'][:2]}"
        qd=quarterly_payments.get(qk,{})
        paid=qd.get("total",0)
        tax=state["tax"]
        owed=max(0,tax["total_tax"]-paid)
        if owed<=0: continue
        if days_left==30:
            ntfy(f"📅 Tax Due in 30 Days",
                 f"Quarter: {q['quarter']}\nDue: {q['due']}\nEstimated owed: ${owed:,.2f}\n"
                 f"Pay at: irs.gov/payments + tax.ny.gov + nyc.gov/finance",
                 tags="calendar")
        elif days_left==7:
            ntfy(f"⚠️ Tax Due in 7 Days",
                 f"Quarter: {q['quarter']}\nDue: {q['due']}\nOwed: ${owed:,.2f}",
                 priority="high",tags="warning")
        elif days_left==1:
            ntfy(f"🚨 Tax Due TOMORROW",
                 f"Quarter: {q['quarter']}\nOwed: ${owed:,.2f}\nPay TODAY\n"
                 f"irs.gov/payments | tax.ny.gov | nyc.gov/finance",
                 priority="urgent",tags="rotating_light")
...
def calc_tax(gross_pnl):
    if gross_pnl<=0:
        return {"gross":gross_pnl,"ltcg":gross_pnl*SEC1256_LTCG,
                "stcg":gross_pnl*SEC1256_STCG,"fed_ltcg":0,"fed_stcg":0,
                "fed_total":0,"ny":0,"nyc":0,"total":0,"net":gross_pnl,"rate":0}
...
def record_tax(asset,direction,entry,exit_p,size,pnl,entry_time):
    ...
    with lock:
        ...
        state["tax"]["total_tax"]+=tax["total"]
        state["tax"]["total_net"]+=tax["net"]
        state["tax"]["total_trades"]+=1
        if pnl>0: state["tax"]["winning_trades"]+=1
        else:      state["tax"]["losing_trades"]+=1
    year=datetime.now(timezone.utc).year
    fname=f"hl_tax_{year}.csv"
    fe=os.path.exists(fname)
    q=get_quarter(datetime.now(timezone.utc))
    row={
    ...
    with open(fname,"a",newline="") as f:
        import csv as _c
        w=_c.DictWriter(f,fieldnames=list(row.keys()))
...
positions={}; last_candle={}; last_exit={}; bar_count={}
entry_times={}
...
def atr_lookup(candles):
    """Precompute O(1) ATR regime lookup"""
    ...
    if len(valid)<50: return [None]*len(candles)
    ma_list=sma(valid,50)
    lookup=[None]*len(candles); vi=0; mi=0
...
def check_signal(candles,asset):
    ...
    if cfg["bb"]:
        ...
        if d=="LONG"  and closes[i]<=u[i]: return None,None,0,0
        if d=="SHORT" and closes[i]>=l[i]: return None,None,0,0
    if cfg["sc"]:
    ...
    if cfg["regime"]:
        try:
            ...
            if lkp[i] and atr_v[i] and atr_v[i]<=lkp[i]*1.2:
...
def enter_trade(asset,direction,price,vol,vs,ef,es):
    ...
    if DRY_RUN:
        log(f"[DRY] ENTER {direction} {asset} @ ${price:,.2f} | size={qty} | liq=${liq:,.2f}")
        ...
        if r and r.get("status")=="ok":
            ...
            if not confirmed:
                add_diag("ERROR",f"Entry NOT confirmed {asset}",
                         "Order placed but position not visible","NOT logging")
                return
...
def exit_trade(asset,price,reason):
    ...
    etime=entry_times.get(asset,ts())
        ...
        if r and r.get("status")=="ok":
            ...
            if not closed:
                add_diag("CRITICAL",f"Exit NOT confirmed {asset}",
                         "Close placed but position still visible","Manual check required")
        if closed:
            if cfg["exit"]=="partial":
...
# ══════════════════════════════════════════════════
def trading_loop():
    log("HL TRADER FINAL — 8 assets | ntfy alerts | execution verified")
    add_diag("INFO","HL Trader Final started",
             f"DRY={DRY_RUN} TEST={TESTNET} LEV={LEVERAGE}x ASSETS={len(ASSETS)}",
             "All orders verified on exchange before logging")
    ntfy("🚀 HL Trader Started",
    ...
    while True:
        ...
        for asset in ASSETS:
            ...
            try:
                ...
                if cfg["ff"]:
                    ...
                    if fr>cfg["ff"]: log(f"⏸  {asset}: funding too high"); continue
                ...
                # ENTRIES
                    ...
                    cd=cfg.get("cd",0)
                    if cd>0 and (bar_count.get(asset,0)-last_exit.get(asset,0))<cd: continue
                    if direction:
        ...
        log(f"💤 Cycle #{cycle} done | next in {CHECK_EVERY}s")
...
def build_dashboard():
    ...
    for asset,pos in s["positions"].items():
        ...
        liq=pos.get("liq",0)
        dist=abs(cur-liq)/liq*100 if liq>0 else 0
    ...
    tax_due_html=""
    if q_info:
        owed=max(0,tax["total_tax"]-quarterly_payments.get("current",{}).get("total",0))
        urgency_color="#FF4757" if days_left<=7 else "#FFB800" if days_left<=30 else "#00D68F"
        tax_due_html=f'''<div style="background:#0F1520;border:2px solid {urgency_color};border-radius:16px;padding:16px;margin-bottom:12px">
          <div style="font-size:10px;font-weight:700;color:#4A5878;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Next Tax Payment Due</div>
          <div style="font-family:monospace;font-size:20px;font-weight:700;color:{urgency_color}">{q_info["quarter"]} — {q_info["due"]}</div>
          <div style="font-size:13px;color:#4A5878;margin-top:4px">{days_left} days remaining · {q_info["period"]}</div>
          <div style="margin-top:10px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;text-align:center">
            <div style="background:#161E2E;border-radius:8px;padding:8px"><div style="font-size:10px;color:#4A5878">Federal</div><div style="font-family:monospace;font-weight:700;color:#FF4757">${tax["total_tax"]*0.707:.2f}</div></div>
            <div style="background:#161E2E;border-radius:8px;padding:8px"><div style="font-size:10px;color:#4A5878">NY State</div><div style="font-family:monospace;font-weight:700;color:#FFB800">${tax["total_tax"]*0.185:.2f}</div></div>
            <div style="background:#161E2E;border-radius:8px;padding:8px"><div style="font-size:10px;color:#4A5878">NYC</div><div style="font-family:monospace;font-weight:700;color:#3D9EFF">${tax["total_tax"]*0.108:.2f}</div></div>
          </div>
        </div>'''
...
<div class="hd">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
    <div style="font-family:monospace;font-size:18px;font-weight:700;color:#00D68F">HL TRADER</div>
...
<div id="ov2" class="sec active">
  ...
  <div class="card" style="border-color:{'rgba(0,214,143,0.3)' if tax['total_net']>=0 else 'rgba(255,71,87,0.3)'}">
    <div style="font-size:10px;font-weight:700;color:#4A5878;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Net P&L (after tax)</div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
    ...
    <div class="card"><div style="font-size:10px;color:#4A5878;font-weight:600;text-transform:uppercase;letter-spacing:.6px;margin-bottom:6px">Win Rate</div><div style="font-family:monospace;font-size:18px;font-weight:700;color:#00D68F">{wr}</div></div>
  </div>
...
<div id="pos" class="sec">
  {pos_html or '<div style="text-align:center;padding:48px 24px;color:#4A5878"><div style="font-size:36px;margin-bottom:12px">📭</div><div>No open positions</div><div style="font-size:12px;margin-top:6px">Waiting for signals...</div></div>'}
</div>
...
<div id="tx" class="sec">
  ...
  <div class="card" style="border-color:rgba(255,184,0,0.3)">
    <div style="font-size:10px;font-weight:700;color:#4A5878;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px">Tax Set-Aside</div>
  ...
  <div class="card" style="padding:0">
    ...
    <div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #1E2D42"><span style="color:#4A5878">Gross P&L</span><span style="font-family:monospace;font-weight:600;color:{'#00D68F' if tax['total_pnl']>=0 else '#FF4757'}">${tax["total_pnl"]:+.2f}</span></div>
    <div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #1E2D42"><span style="color:#4A5878">60% LTCG portion</span><span style="font-family:monospace">${tax["total_pnl"]*0.6:.2f}</span></div>
    <div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #1E2D42"><span style="color:#4A5878">40% STCG portion</span><span style="font-family:monospace">${tax["total_pnl"]*0.4:.2f}</span></div>
    <div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #1E2D42"><span style="color:#4A5878">Federal tax (~26.8%)</span><span style="font-family:monospace;color:#FF4757">-${tax["total_tax"]*0.707:.2f}</span></div>
    ...
    <div style="display:flex;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #1E2D42"><span style="color:#4A5878">NYC Local (3.876%)</span><span style="font-family:monospace;color:#FF4757">-${tax["total_tax"]*0.108:.2f}</span></div>
  ...
  <div style="font-size:11px;color:#4A5878;text-align:center;padding:16px 0;line-height:1.6">Section 1256 · 60/40 rule · Estimate only<br>Consult a CPA for final filing</div>
</div>
...
<div id="dg" class="sec">
  ...
  <div class="card" style="padding:0 16px">
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #1E2D42">
      ...
      <div style="flex:1"><div style="font-size:13px;font-weight:600">Strategy Loop</div><div style="font-size:11px;color:#4A5878;font-family:monospace">Cycle #{s["cycle"]} · {status}</div></div>
...
def index():
    if not session.get("ok"):
        ...
        <div style="color:#4A5878;font-size:13px;margin-bottom:32px">8 Assets · HyperLiquid</div>
...
def test_suite():
    ...
    for asset in ASSETS:
        ...
        check(f"Candles + freshness: {asset}",t3)

    def t4():
        ok=state["cycle"]>0; return ok,f"Cycle #{state['cycle']} | {state['status']}"
    ...
    def t6():
        r=exchange.update_leverage(LEVERAGE,"BTC",True); return r is not None,f"{LEVERAGE}x confirmed"
    check("Leverage setting works",t6)
    ...
    def t9():
        r=req.post(NTFY_URL,data="✅ HL Trader test alert — ntfy working",
                   headers={"Title":"System Test","Tags":"white_check_mark"},timeout=5)
        return r.status_code==200,f"HTTP {r.status_code} — check your phone ✅"
    check("ntfy alert delivered",t9)

    def t10():
        if not positions: return True,"No open positions — N/A"
        details=[]
        for asset,pos in positions.items():
            liq=liq_price(pos["entry"],pos["direction"])
            cur=pos.get("current_price",pos["entry"])
            dist=abs(cur-liq)/liq*100
            details.append(f"{asset} liq@${liq:,.2f} ({dist:.1f}% away)")
        return True," | ".join(details)
    check("Liquidation prices calculated",t10)
    ...
    <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #1E2D42">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:#4A5878;font-family:monospace;margin-top:2px">{r["detail"]}</div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        ...
        <div style="font-size:10px;color:#4A5878;margin-top:2px">{r["ms"]}ms</div>
      </div>
    </div>''' for r in results)

    return f'''<!DOCTYPE html><html><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<meta name="apple-mobile-web-app-capable" content="yes"><title>System Test</title></head>
...
<div style="max-width:600px;margin:0 auto">
  ...
  <div style="background:{"rgba(0,214,143,0.1)" if all_pass else "rgba(255,71,87,0.1)"};border:2px solid {"#00D68F" if all_pass else "#FF4757"};border-radius:16px;padding:20px;text-align:center;margin-bottom:20px">
    ...
    <div style="font-size:13px;color:#4A5878;margin-top:6px">{"🚀 All systems go" if all_pass else "⚠️ Fix failing tests before going live"}</div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
    ...
    <a href="/force-trade" style="display:block;text-align:center;background:#0F1520;border:1px solid #FFB800;border-radius:12px;padding:14px;color:#FFB800;font-size:13px;font-weight:600;text-decoration:none">⚡ Force Trade</a>
  </div>
</div></body></html>'''
...
def force_trade():
    ...
    price=0; qty=0; fill_price=0; close_price=0
    ...
    def s1():
        nonlocal price
        mids=info.all_mids(); price=float(mids.get(asset,0)); return price>0,f"${price:,.2f}"
    step("Get current BTC price",s1)
    ...
    def s2():
        ...
        return qty>0,f"{qty} BTC (${qty*price:.2f} notional)"
    step("Calculate position size",s2)
    ...
    def s4():
        ...
        return False,"Position NOT visible on exchange"
    step("Verify position on exchange",s4)

    def s5():
        time.sleep(30)
        mids=info.all_mids(); cur=float(mids.get(asset,fill_price))
        pnl=(cur-fill_price)*qty; return True,f"Held 30s | cur=${cur:,.2f} | P&L=${pnl:+.4f}"
    step("Hold position 30 seconds",s5)
    ...
    def s7():
        ...
        closed=len(still)==0; return closed,"Confirmed closed ✅" if closed else "Still open ❌"
    step("Verify closed on exchange",s7)
    ...
    step("Calculate final P&L",s8)

    def s9():
        passed=sum(1 for r in results if r["ok"])
        pnl=(close_price-fill_price)*qty if fill_price and close_price else 0
        r=req.post(NTFY_URL,data=f"Force trade: {passed}/{len(results)} steps passed | P&L: ${pnl:+.4f}",
                   headers={"Title":"Force Trade Test","Tags":"test_tube"},timeout=5)
        return r.status_code==200,"Alert sent to phone ✅"
    step("Send ntfy confirmation",s9)
    ...
    return f'''<!DOCTYPE html><html><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<meta name="apple-mobile-web-app-capable" content="yes"><title>Force Trade Test</title></head>
<body style="background:#080B10;color:#E8EDF5;font-family:-apple-system,sans-serif;padding:20px;padding-top:calc(20px + env(safe-area-inset-top))">
<div style="max-width:600px;margin:0 auto">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
  ...
  <div style="background:{"rgba(0,214,143,0.1)" if all_pass else "rgba(255,71,87,0.1)"};border:2px solid {"#00D68F" if all_pass else "#FF4757"};border-radius:16px;padding:20px;text-align:center;margin-bottom:20px">
    <div style="font-size:40px;margin-bottom:8px">{"✅" if all_pass else "❌"}</div>
    <div style="font-family:monospace;font-size:24px;font-weight:700;color:{"#00D68F" if all_pass else "#FF4757"}">{passed}/{total} STEPS PASSED</div>
    <div style="font-size:13px;color:#4A5878;margin-top:6px">{"Full execution confirmed ✅" if all_pass else "Execution issue detected"}</div>
  </div>
  <div style="background:#0F1520;border:1px solid #1E2D42;border-radius:16px;padding:0 16px;margin-bottom:16px">{rows_html}</div>
...
def signal_check():
    ...
    for asset in ASSETS:
        try:
            ...
            cur=float(candles[-1]["c"])
            closes=[float(c["c"]) for c in candles]; highs=[float(c["h"]) for c in candles]
            lows=[float(c["l"]) for c in candles]; vols=[float(c["v"]) for c in candles]
            ef=ema(closes,EMA_FAST); em2=ema(closes,EMA_MID); es_arr=ema(closes,EMA_SLOW)
            vs=sma(vols,20); i=len(candles)-1
            filters=[]; blocked=[]
            if ef[i] and em2[i] and es_arr[i]:
                if   ef[i]>em2[i]>es_arr[i]: d="LONG"
                elif ef[i]<em2[i]<es_arr[i]: d="SHORT"
                else: d=None
            else: d=None
            filters.append(f"EMA stack: {'✅ ' + (d or 'aligned') if d else '❌ not aligned'}")
            if not d: blocked.append("EMA not stacked")
            if d and es_arr[i]:
                sep=abs(ef[i]-es_arr[i])/es_arr[i]; ok_sep=sep>=SEP_FILTER
                filters.append(f"Separation: {'✅' if ok_sep else '❌'} {sep:.4f}")
                if not ok_sep: blocked.append("separation too tight")
            if d and vs[i]:
                vr=vols[i]/vs[i]; ok_vol=vr>=VOL_FILTER
                filters.append(f"Volume: {'✅' if ok_vol else '❌'} {vr:.1f}x")
                if not ok_vol: blocked.append("volume too low")
            if d and i>=BRK_BARS:
                brk=closes[i]>max(highs[i-BRK_BARS:i]) if d=="LONG" else closes[i]<min(lows[i-BRK_BARS:i])
                filters.append(f"Breakout: {'✅' if brk else '❌'}")
                if not brk: blocked.append("no breakout")
            results.append({"asset":asset,"signal":d if not blocked else None,"price":cur,"filters":filters,"blocked_by":blocked,"direction":d})
        except Exception as e:
            results.append({"asset":asset,"signal":None,"price":0,"filters":[],"blocked_by":[str(e)]})

    firing=[r for r in results if r.get("signal")]
    rows_html="".join(f'''
    <div style="background:#161E2E;border:1px solid #1E2D42;border-radius:14px;padding:14px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <div style="font-family:monospace;font-size:15px;font-weight:700">{r["asset"]}-PERP</div>
        <div style="display:flex;align-items:center;gap:8px">
          <span style="font-family:monospace;font-size:13px">${r["price"]:,.2f}</span>
          <span style="font-size:12px;font-weight:700;padding:3px 10px;border-radius:6px;background:rgba({("0,214,143" if r.get("signal")=="LONG" else "255,71,87" if r.get("signal")=="SHORT" else "74,88,120")},0.2);color:{"#00D68F" if r.get("signal")=="LONG" else "#FF4757" if r.get("signal")=="SHORT" else "#4A5878"}">{r.get("signal") or "NO SIGNAL"}</span>
        </div>
      </div>
      {"".join(f"<div style='font-size:11px;color:#4A5878;font-family:monospace;margin-top:2px'>{f}</div>" for f in r["filters"])}
      {f"<div style='font-size:11px;color:#FF4757;margin-top:4px'>Blocked: {', '.join(r['blocked_by'])}</div>" if r.get("blocked_by") else ""}
    </div>''' for r in results)

    return f'''<!DOCTYPE html><html><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<title>Signal Check</title></head>
<body style="background:#080B10;color:#E8EDF5;font-family:-apple-system,sans-serif;padding:20px;padding-top:calc(20px + env(safe-area-inset-top))">
<div style="max-width:600px;margin:0 auto">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
    <a href="/" style="color:#4A5878;text-decoration:none;font-size:13px">← Dashboard</a>
  ...
  <div style="background:#0F1520;border:1px solid #1E2D42;border-radius:16px;padding:16px;text-align:center;margin-bottom:16px">
    ...
    <div style="font-family:monospace;font-size:16px;font-weight:700;color:{"#00D68F" if firing else "#4A5878"}">
...
def tax_guide():
    ...
    return f'''<!DOCTYPE html><html><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<title>Tax Payment Guide</title></head>
<body style="background:#080B10;color:#E8EDF5;font-family:-apple-system,sans-serif;padding:20px;padding-top:calc(20px + env(safe-area-inset-top))">
<div style="max-width:600px;margin:0 auto">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
    <a href="/" style="color:#4A5878;text-decoration:none;font-size:13px">← Dashboard</a>
  ...
  <div style="background:#0F1520;border:1px solid #FFB800;border-radius:16px;padding:16px;margin-bottom:16px">
    <div style="font-size:12px;color:#FFB800;font-weight:700;margin-bottom:8px">NEXT DUE DATE</div>
  ...
  <div style="background:#0F1520;border:1px solid #1E2D42;border-radius:16px;padding:16px;margin-bottom:16px">
    ...
    <div style="padding:12px 0;border-bottom:1px solid #1E2D42">
      <div style="font-weight:600;margin-bottom:4px">1. Federal (IRS)</div>
      <div style="font-size:13px;color:#4A5878">Go to irs.gov/payments → Direct Pay → Select "Estimated Tax"</div>
    </div>
    <div style="padding:12px 0;border-bottom:1px solid #1E2D42">
      <div style="font-weight:600;margin-bottom:4px">2. NY State</div>
      <div style="font-size:13px;color:#4A5878">Go to tax.ny.gov → Make a Payment → Estimated Tax</div>
    </div>
    <div style="padding:12px 0">
      <div style="font-weight:600;margin-bottom:4px">3. NYC Local</div>
      <div style="font-size:13px;color:#4A5878">Go to nyc.gov/finance → NYC Estimated Tax</div>
    </div>
  </div>
  <div style="background:#0F1520;border:1px solid #1E2D42;border-radius:16px;padding:16px;margin-bottom:16px">
    <div style="font-size:12px;color:#4A5878;font-weight:700;margin-bottom:12px">QUARTERLY SCHEDULE</div>
    {"".join(f"<div style='display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #1E2D42'><span style='color:#4A5878'>{qd['quarter']}</span><div style='text-align:right'><div style='font-family:monospace;font-weight:600'>{qd['due']}</div><div style='font-size:11px;color:#4A5878'>{qd['period']}</div></div></div>" for qd in QUARTERLY_DATES[:4])}
  </div>
  <div style="background:#0F1520;border:1px solid #1E2D42;border-radius:16px;padding:16px">
    ...
    <div style="font-size:13px;color:#4A5878;line-height:1.8">
      60% treated as long-term gains (20% federal)<br>
      40% treated as short-term gains (37% federal)<br>
      Blended federal rate: ~26.8%<br>
      NY State: 6.85%<br>
      NYC Local: 3.876%<br>
      Total effective rate: ~37-38%
    </div>
  </div>
</div></body></html>'''
...
def log_export():
    if not session.get("ok"): return "unauthorized",401
    s=state; lines=["="*60,"HL TRADER — SYSTEM LOG",f"Generated: {ts()} UTC","="*60]
    lines.append(f"\nSTATUS: {s['status']} | Cycle #{s['cycle']} | {s['leverage']}x | {'DRY' if s['dry_run'] else 'LIVE'} | {'Testnet' if s['testnet'] else 'Mainnet'}")
